...
const EditCommentModal = ({ open, comment, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-2xl max-w-lg w-full overflow-hidden relative z-10 animate-[scaleUp_0.2s_ease-out]">
        {/* Top Accent Bar */}
        <div className="h-1.5 bg-gradient-to-r from-purple-600 via-fuchsia-500 to-indigo-600" />

        {/* Modal Header */}
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-br from-purple-50/60 via-white to-indigo-50/30">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white flex items-center justify-center text-xl shadow-md shadow-purple-500/20 shrink-0">
              <FaEdit />
            </div>
            <div>
              ...
              <p className="text-xs text-slate-500 font-semibold">
                Modify user comment text for content moderation
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-left">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full rounded-2xl border border-slate-200 bg-slate-50/80 p-4 text-xs font-semibold text-slate-800 focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/15 outline-none transition-all shadow-inner resize-none"
            />
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-6 py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-extrabold text-xs uppercase tracking-wider shadow-md shadow-purple-500/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {saving ? (
                <>
